...
class ChangePasswordServices {
  public sendCode = async (email: string) => {
    try {
      const response = await ApiIntance.post(
        ConsultationsPaths.FORGOT_PASSWORD,
        { email }
      );
  ...
  public verifyCode = async (code: string) => {
    try {
      const response = await ApiIntance.post(ConsultationsPaths.VERIFY_CODE, {
        code,
      });
      return response.data;
    } catch (error) {
  ...
  public resetPassword = async (password: string) => {
    try {
      const response = await ApiIntance.put(ConsultationsPaths.RESET_PASSWORD, {
        password,
      });
      return response.data;
    } catch (error) {
